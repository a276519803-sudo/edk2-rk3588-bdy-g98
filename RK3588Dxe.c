  ...
  )
{
  ...
  for (Index = 0; Index < ARRAY_SIZE (ComPhyMode); Index++) {
    if (ComPhyMode[Index] == COMBO_PHY_MODE_SATA) {
      /* Enable SATA ACLK and deassert reset (v2.5 conservative config) */
      switch (Index) {
        case 0:
          HAL_CRU_ClkEnable (ACLK_SATA0_GATE);
          HAL_CRU_RstAssert (SRST_A_SATA0);
          gBS->Stall (10);
          HAL_CRU_RstDeassert (SRST_A_SATA0);
          PcieSlotIndex = PCIE_SEGMENT_PCIE20L2;
          break;
        case 1:
          HAL_CRU_ClkEnable (ACLK_SATA1_GATE);
          HAL_CRU_RstAssert (SRST_A_SATA1);
          gBS->Stall (10);
          HAL_CRU_RstDeassert (SRST_A_SATA1);
          PcieSlotIndex = PCIE_SEGMENT_PCIE20L0;
          break;
        case 2:
          HAL_CRU_ClkEnable (ACLK_SATA2_GATE);
          HAL_CRU_RstAssert (SRST_A_SATA2);
          gBS->Stall (10);
          HAL_CRU_RstDeassert (SRST_A_SATA2);
